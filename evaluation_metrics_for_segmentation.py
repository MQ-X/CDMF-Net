...
from .file_management import get_filenames, save_csv_mean_segmentation_performance, save_csv_segmentation_table
...
def generate_table_of_results(image_filenames, segmentation_folder, gt_folder, is_training=False):
    ...
    for i in range(len(image_filenames)):
        ...
        if is_training:
            ...
            gt_filename = path.join(gt_folder, image_filenames[i].replace('bmp', 'png'))
            if path.exists(gt_filename):
                gt_label = np.array(
                    Image.fromarray(imageio.imread(gt_filename, pilmode='L')).resize(segmentation.shape[:2]
                                                                                     ,resample=Image.NEAREST))  # pay attention to the size parameter
    # for i in range(len(image_filenames)):
    #
    #     # read the segmentation
    #     segmentation = imageio.imread(path.join(segmentation_folder, image_filenames[i]))  # misc->imageio
    #     if len(segmentation.shape) > 2:
    #         segmentation = segmentation[:, :, 0]
    #     # read the gt
    #     if is_training:
    #         gt_filename = path.join(gt_folder, 'Glaucoma', image_filenames[i])
    #         if path.exists(gt_filename):
    #             gt_label = imageio.imread(gt_filename)  # misc->imageio
    #         else:
    #             gt_filename = path.join(gt_folder, 'Non-Glaucoma', image_filenames[i])
    #             if path.exists(gt_filename):
    #                 gt_label = imageio.imread(gt_filename)  # misc->imageio
    ...
    #                                                                                  resample=Image.NEAREST))  # pay attention to the size parameter
            ...
            # gt_label = misc.imresize(imageio.imread(gt_filename, mode='L'), size=segmentation.shape[:2],
            # interp='nearest')  # pay attention to the size parameter
            else:
                raise ValueError(
...
#results/baseline/refuge/pred_mask
#./fundus/refuge/test/mask
def evaluate_segmentation_results(segmentation_folder, gt_folder, output_path=None, export_table=False,
    ...
    if not (output_path is None) and (export_table):
        ...
        table_filename = path.join(output_path, 'RIM-ONE-r3.csv')
        # save the table
    ...
    #                                                                        str(mae_cdr)))
    print('CDR = {}'.format(str(mae_cdr)))
    # save the mean values in the output path
    if not (output_path is None):
        # initialize the output filename
        output_filename = path.join(output_path, 'n_evaluation_segmentation.csv')
        # save the results
