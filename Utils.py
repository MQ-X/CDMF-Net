...
def draw_ent(prediction, save_root, name):
    ...
    name0 = name.split('.')[0].split('\\')[4]  #提取图片名字
    cv2.imwrite(os.path.join(save_root, 'disc', name0) + '.png', disc)
    cup = construct_color_img(cup_ent)
    cv2.imwrite(os.path.join(save_root, 'cup', name0) + '.png', cup)
...
def draw_mask(prediction, save_root, name):
   ...
   '''
    ...
    if not os.path.exists(os.path.join(save_root, 'cup')):
        os.makedirs(os.path.join(save_root, 'cup'))
    if not os.path.exists(os.path.join(save_root, 'cup+disc')):
        os.makedirs(os.path.join(save_root, 'cup+disc'))
    cup = prediction[0]
    disc = prediction[1]

    disc = construct_color_img(disc)
    cv2.imwrite(os.path.join(save_root, 'disc',name.split('.')[0].split('\\')[4]) + '.png', disc)
    cup = construct_color_img(cup)
    cv2.imwrite(os.path.join(save_root, 'cup', name.split('.')[0].split('\\')[4]) + '.png', cup)
    Get = disc+cup
    cv2.imwrite(os.path.join(save_root, 'cup+disc', name.split('.')[0].split('\\')[4]) + '.png', Get)
...
def postprocessing(prediction, threshold=0.75, dataset='G'):
    if dataset[0] == 'D':
        ...
        # disc_mask = morphology.binary_dilation(disc_mask, morphology.diamond(7)).astype(np.uint8)
        # cup_mask = morphology.binary_dilation(cup_mask, morphology.diamond(7)).astype(np.uint8)
