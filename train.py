...
def main():
    ...
    parser.add_argument(
        '--dataset', type=str, default='RIM-ONE-r3', help='folder id contain images ROIs to train or validation'
    )              #gdrishtiGS
    parser.add_argument(
    ...
    # 2. model
    #model_gen = DeepLab(num_classes=2, backbone='mobilenetv3', output_stride=args.out_stride,
      #                   sync_bn=args.sync_bn, freeze_bn=args.freeze_bn).cuda()
    #model_gen = Net(num_classes=2, aux=args.aux).cuda()
    model_gen = FastSCNN_NEW3(num_classes=2, aux=args.aux).cuda()
    #model_gen = ENet(num_classes=2, aux=args.aux).cuda()
