...
from networks.LEDnet import *
# from networks.fast_scnn_new1 import *
...
def main():
    ...
    parser.add_argument('--model-file', type=str,
                        default='./logs/RIM-ONE-r3/allin/'
                                'checkpoint_688.pth.tar',
                        help='Model path')
    parser.add_argument(
        '--dataset', type=str, default='RIM-ONE-r3', help='test folder id contain images ROIs to test'
    )
    parser.add_argument(
        '--aux', type=str, default=False, help='test folder id contain images ROIs to test'
    )
    ...
    parser.add_argument(
        ...
        default='./fundus/RIM-ONE-r3/test/mask',
    ...
    # 2. model
    # model = DeepLab(num_classes=2, backbone='mobilenetv3', output_stride=args.out_stride,
    ...
    test_cup_dice_iou = 0.0
    test_disc_dice_iou = 0.0

    test_cup_dice_sen = 0.0
    test_disc_dice_sen = 0.0

    # test_cup_dice_pre = 0.0
    # test_disc_dice_pre = 0.0
    ...
    with torch.no_grad():
        for batch_idx, (sample) in tqdm.tqdm(enumerate(test_loader),
            ...
            print(cup_dice[0], disc_dice[0])
            test_cup_dice += cup_dice[0]
            test_disc_dice += disc_dice[0]

            print(cup_dice[1], disc_dice[1])
            test_cup_dice_iou += cup_dice[1]
            test_disc_dice_iou += disc_dice[1]

            print(cup_dice[2], disc_dice[2])
            test_cup_dice_sen += cup_dice[2]
            test_disc_dice_sen += disc_dice[2]

            # print(cup_dice[3], disc_dice[3])
            # test_cup_dice_pre += cup_dice[3]
            # test_disc_dice_pre += disc_dice[3]
            #
        ...
        test_cup_dice_iou /= len(test_loader)
        test_disc_dice_iou /= len(test_loader)

        test_cup_dice_sen /= len(test_loader)
        test_disc_dice_sen /= len(test_loader)

        # test_cup_dice_pre /= len(test_loader)
        # test_disc_dice_pre /= len(test_loader)

        print("test_cup_dice = ", test_cup_dice)
        print("test_disc_dice = ", test_disc_dice)

        print("test_cup_dice_iou = ", test_cup_dice_iou)
        print("test_disc_dice_iou = ", test_disc_dice_iou)

        print("test_cup_dice_sen = ", test_cup_dice_sen)
        print("test_disc_dice_sen = ", test_disc_dice_sen)

        # print("test_cup_dice_pre = ", test_cup_dice_pre)
        # print("test_disc_dice_pre = ", test_disc_dice_pre)
    ...
    _, _, mae_cdr, _ = evaluate_segmentation_results(
        ...
        # osp.join(args.test_prediction_save_path, args.dataset, 'pred_mask'),
        osp.join(args.test_prediction_save_path, args.dataset, 'pred_mask/RIM-ONE-r3/test/ROIs/image'),
